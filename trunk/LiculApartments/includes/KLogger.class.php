<?php	
	class KLogger
	{
		...
		public function __construct( $filepath , $priority )
		{
			...
			$this->log_file = $filepath . "log_" . date("Y_m_d") . ".log";
		...
		public function shouldLogDebug()
		{
			return $this->shouldLog(KLogger::DEBUG);
		}
		
		public function shouldLogInfo()
		{
			return $this->shouldLog(KLogger::INFO);
		}
		
		public function shouldLogWarn()
		{
			return $this->shouldLog(KLogger::WARN);
		}
		
		public function shouldLogError()
		{
			return $this->shouldLog(KLogger::ERROR);
		}
		
		public function shouldLogFatal()
		{
			return $this->shouldLog(KLogger::FATAL);
		}
		
		private function shouldLog($priority)
		{
		...
		public function LogFatalArray($line)
		{
			$this->Log( $line , KLogger::FATAL );
		}
		...
		public function Log($line, $priority)
		{
			if ($this->shouldLog($priority))
			{
				...
				if (is_array($line))
				{
					foreach ($line as $key => $message) 
					{						
						$this->WriteFreeFormLine ( "$status $key => $message \n" );
					}
				}
				else
				{					
